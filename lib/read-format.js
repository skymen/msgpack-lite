...
function getReadFormat(options) {
  ...
  var readFormat = {
    ...
    ext: ext,
    // Smaller integer types always decode as Number
    // Only int64/uint64 decode as BigInt when bigint option is set
    uint8: uint8,
    uint16: uint16,
    uint32: uint32,
    uint64: read(
      8,
      bigint ? readUInt64BE_bigint : int64 ? readUInt64BE_int64 : readUInt64BE
    ),
    int8: int8,
    int16: int16,
    int32: int32,
    int64: read(
      8,
      bigint ? readInt64BE_bigint : int64 ? readInt64BE_int64 : readInt64BE
    ),
...
function readUInt64BE_bigint(start) {
  ...
  var value = BigInt(0);
  for (var i = 0; i < 8; i++) {
    value = (value << BigInt(8)) | BigInt(buffer[start + i]);
  }
  return value;
}
